package com.keydom.mianren.ih_patient.view.baidu;
...
public class PosIndicator {
    ...
    public void savePosOnConfigurationChanged() {
        ...
        if (DEBUG) {
            Log.d(TAG, "savePosOnConfigurationChanged, mSaveCurrentPosRation: " + mSaveCurrentPosRatio);
        }
    ...
    private void restorePosIfNeeded() {
        if (mConfigurationHasChanged) {
            ...
            if (DEBUG) {
                Log.d(TAG, "Need restore current pos, mCurrentPos: " + mCurrentPos);
            }
        }
    }
