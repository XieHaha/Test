...
public class HospitalPaymentController extends ControllerImpl<HospitalPaymentView> implements View.OnClickListener {
    ...
    public void getHospitalPayment() {
        ApiRequest.INSTANCE.request(HttpService.INSTANCE.createService(HospitalPaymentService.class).getInHospitalRecord(getView().getMedicalCardNumber()), new HttpSubscriber<List<MedicalCardInfo>>(getContext(), getDisposable(), true) {
            @Override
            public void requestComplete(@Nullable List<MedicalCardInfo> data) {
            }
