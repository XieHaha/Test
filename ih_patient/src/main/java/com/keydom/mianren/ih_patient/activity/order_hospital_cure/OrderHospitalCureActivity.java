...
import com.keydom.mianren.ih_patient.activity.order_hospital_cure.controller.OrderHospitalCureController;
import com.keydom.mianren.ih_patient.activity.order_hospital_cure.view.OrderHospitalCureView;
...
public class OrderHospitalCureActivity extends BaseControllerActivity<OrderHospitalCureController> implements OrderHospitalCureView {
    ...
    private InterceptorEditText order_name_edt, order_phone_edt, relationshiper_phone_edt,
            order_address_edt, cure_service_edt, health_care_card_num, order_note_edt;
    private TextView order_region_tv, order_date_tv, order_health_care_type_tv,
            order_health_care_region_tv, order_settlement_type_tv, click_to_show_more_tv,
            forecast_ordered_time_tv, cure_attention_tv, remove_order_tv, commit_order_tv,
            unconfirm_tv, confirm_tv, confirm_head_label_tv;
    private InterceptorEditText apply_date_edt, report_date_edt, report_address_edt,
            report_kown_edt;
    private RelativeLayout click_to_show_more_layout, order_audit_head_layout,
            order_confirm_head_layout;
    private LinearLayout confirm_order_layout, order_top_layout, order_bottom_layout,
            heal_care_layout, report_layout;
    ...
    public void initData(@Nullable Bundle savedInstanceState) {
        ...
        if (type.equals(Type.ORDERCUREAUDIT)) {
            ...
            LinearLayout.LayoutParams layoutParams =
                    (LinearLayout.LayoutParams) cure_attention_tv.getLayoutParams();
            layoutParams.setMargins(0, getResources().getDimensionPixelSize(R.dimen.dp_43), 0,
                    getResources().getDimensionPixelSize(R.dimen.dp_43));
    ...
    @Override
    public void getHealthCureSuccess(com.keydom.mianren.ih_patient.bean.HospitalCureInfo hospitalCureInfo) {
        ...
        if (queryHospitalCureInfo.getApplyTime() != null && !"".equals(queryHospitalCureInfo.getApplyTime()))
            order_time_tv.setText(DateUtils.getDate(queryHospitalCureInfo.getApplyTime(),
                    DateUtils.YYYY_MM_DD_HH_MM_SS));
        if (queryHospitalCureInfo.getApprovalTime() != null && !"".equals(queryHospitalCureInfo.getApprovalTime()))
            audit_time_tv.setText(DateUtils.getDate(queryHospitalCureInfo.getApprovalTime(),
                    DateUtils.YYYY_MM_DD_HH_MM_SS));
        if (queryHospitalCureInfo.getConfirmationTime() != null && !"".equals(queryHospitalCureInfo.getConfirmationTime()))
            confirm_time_tv.setText(DateUtils.getDate(queryHospitalCureInfo.getConfirmationTime()
                    , DateUtils.YYYY_MM_DD_HH_MM_SS));
        ...
        if (queryHospitalCureInfo.getState() == 4) {
            ...
            confirm_head_label_tv.setText("您已以“" + refuseReason +
                    "”为由拒绝住院申请，本院将近期致电联系您核实，请注意保持电话畅通");
