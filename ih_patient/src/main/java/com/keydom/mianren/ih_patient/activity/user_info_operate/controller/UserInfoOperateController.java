...
public class UserInfoOperateController extends ControllerImpl<UserInfoOperateView> implements View.OnClickListener {
                        ...
                        getView().getAreaName(), new GeneralCallback.SelectRegionListener() {
                            ...
                            public void getSelectedRegion(List<PackageData.ProvinceBean> data,
                                ...
                                if (data.get(position1).getCity().size() == 0) {
                                    ...
                                    map.put("provinceCode", data.get(position1).getCode());
                                    map.put("cityCode",
                                            data.get(position1).getCity().get(position2).getCode());
                                    upLoadInfo(map);
                                } else {
                                    ...
                                    map.put("cityCode",
                                            data.get(position1).getCity().get(position2).getCode());
                                    map.put("countyCode",
                                            data.get(position1).getCity().get(position2).getArea().get(position3).getCode());
                                    upLoadInfo(map);
                                }
                            }
                        });
            ...
            case R.id.user_nation_tv:
                if (NoFastClickUtils.isFastClick()) {
                    SelectDialogUtils.showNationSelectDialog(getContext(), getView().getNation(),
                            new GeneralCallback.SelectNationListener() {
                                @Override
    ...
    private void showNameEditDialog() {
        ...
        View view = LayoutInflater.from(getContext()).inflate(R.layout.name_edit_dialog_bg, null,
                false);
    ...
    private void upLoadRegion(String provinceCode, String cityCode, String areaCode) {
        ApiRequest.INSTANCE.request(HttpService.INSTANCE.createService(UserService.class).upLoadRegion(String.valueOf(Global.getUserId()), provinceCode, cityCode, areaCode), new HttpSubscriber<Object>(getContext(), getDisposable(), true) {
            @Override
    ...
    public void upLoadInfo(Map<String, Object> map) {
        ApiRequest.INSTANCE.request(HttpService.INSTANCE.createService(UserService.class).upLoadInfo(HttpService.INSTANCE.object2Body(map)), new HttpSubscriber<Object>(getContext(), getDisposable(), true) {
            @Override
            public void requestComplete(@Nullable Object data) {
                //                if (map.containsKey("sex")) {
                //                    ImClient.updateUserInfo(UserInfoFieldEnum.GENDER, Integer
                //                    .valueOf(map.get("sex").toString()), null);
                //                } else if (map.containsKey("userName")) {
                //                    ImClient.updateUserInfo(UserInfoFieldEnum.Name, map.get
    ...
    public void findAllCountry() {
        ApiRequest.INSTANCE.request(HttpService.INSTANCE.createService(UserService.class).getCountryList(), new HttpSubscriber<List<PackageData.CountryBean>>(getContext(), getDisposable(), false) {
            ...
            public void requestComplete(@Nullable final List<PackageData.CountryBean> data) {
                ...
                OptionsPickerView countryPickerView = new OptionsPickerBuilder(getContext(),
                        new OnOptionsSelectListener() {
                            @Override
                            public void onOptionsSelect(int options1, int option2, int options3,
                                                        View v) {
    ...
    public void upLoadPic(String path) {
        ...
        ApiRequest.INSTANCE.request(HttpService.INSTANCE.createService(UploadService.class).upload(body), new HttpSubscriber<String>(getContext(), getDisposable(), true) {
            @Override
