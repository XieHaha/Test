...
public class SwitchButton extends View implements View.OnClickListener {
	...
	private RectF mBgStrokeRectF,mBallStrokeRectF;
	...
	private void initData() {
		...
		greenColor = Color.parseColor("#65F0A8");
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		mViewHeight = h-this.getPaddingBottom()-this.getPaddingTop();
		mViewWidth = w-this.getPaddingLeft()-this.getPaddingRight();
		switchViewStrockWidth =0;
		mStrokeRadius = mViewWidth/3;
	...
	private void drawSwitchBall(Canvas canvas) {
		canvas.drawRoundRect(new RectF(mSwitchBallx-mStrokeRadius,0, mSwitchBallx+mStrokeRadius, mViewHeight), mStrokeRadius, mStrokeRadius, mBallPaint);
	...
	public void setState(boolean isOpen){
		mCurrentState = (isOpen ? State.OPEN : State.CLOSE);
		switchViewBgColor = (isOpen ?greenColor:greyColor);
