...
public class MyRoundRectShape extends RectShape {
    /*
     ...
     */
    public MyRoundRectShape(float[] outerRadii, RectF inset,
                            float[] innerRadii,float measureWidth) {
    ...
    protected void onResize(float w, float h) {
        ...
        RectF r = rect();
        r.left+=measureWidth/2;
        r.top+=measureWidth/2;
        r.right-=measureWidth/2;
        r.bottom-=measureWidth/2;
    ...
    @Override
    public MyRoundRectShape clone() throws CloneNotSupportedException {
        MyRoundRectShape shape = (MyRoundRectShape) super.clone();
