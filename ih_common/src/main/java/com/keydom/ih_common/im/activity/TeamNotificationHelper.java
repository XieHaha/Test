...
public class TeamNotificationHelper {
    ...
    public static String getTeamNotificationText(IMMessage message, String tid) {
        return getTeamNotificationText(message.getSessionId(), message.getFromAccount(),
                (NotificationAttachment) message.getAttachment());
    }

    public static String getTeamNotificationText(String tid, String fromAccount,
    ...
    private static String buildNotification(String tid, String fromAccount,
                                            NotificationAttachment attachment) {
        String text;
        switch (attachment.getType()) {
            case InviteMember:
                text = buildInviteMemberNotification(((MemberChangeAttachment) attachment),
                        fromAccount);
                break;
            case KickMember:
            ...
            case DismissTeam:
                ...
                break;
            case UpdateTeam:
                text = buildUpdateTeamNotification(tid, fromAccount,
                        (UpdateTeamAttachment) attachment);
                break;
            case PassTeamApply:
                text = buildManagerPassTeamApplyNotification((MemberChangeAttachment) attachment);
                break;
            case TransferOwner:
                text = buildTransferOwnerNotification(fromAccount,
                        (MemberChangeAttachment) attachment);
                break;
            case AddTeamManager:
            ...
            case RemoveTeamManager:
                ...
                break;
            case AcceptInvite:
                text = buildAcceptInviteNotification(fromAccount,
                        (MemberChangeAttachment) attachment);
                break;
            case MuteTeamMember:
    ...
    private static String buildInviteMemberNotification(MemberChangeAttachment a,
                                                        String fromAccount) {
        StringBuilder sb = new StringBuilder();
        ...
        Team team = ImClient.getTeamProvider().getTeamById(teamId.get());
    ...
    private static String buildUpdateTeamNotification(String tid, String account,
                                                      UpdateTeamAttachment a) {
        StringBuilder sb = new StringBuilder();
        for (Map.Entry<TeamFieldEnum, Object> field : a.getUpdatedFields().entrySet()) {
            if (field.getKey() == TeamFieldEnum.Name) {
                ...
                //                sb.append(TeamHelper.getTeamMemberDisplayNameYou(tid, account)
                //                + " 修改了群公告");
                ...
                if (type == VerifyTypeEnum.Free) {
                    //                    sb.append(authen + NimUIKit.getContext().getString(R
                    //                    .string.team_allow_anyone_join));
                } else if (type == VerifyTypeEnum.Apply) {
                    //                    sb.append(authen + NimUIKit.getContext().getString(R
                    //                    .string.team_need_authentication));
                } else {
                    //                    sb.append(authen + NimUIKit.getContext().getString(R
                    //                    .string.team_not_allow_anyone_join));
                }
