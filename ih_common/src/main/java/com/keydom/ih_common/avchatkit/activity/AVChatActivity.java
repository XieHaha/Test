    ...
    private void handleCallControl(AVChatControlEvent notification) {
        ...
        switch (notification.getControlCommand()) {
            ...
            case AVChatControlCommand.SWITCH_AUDIO_TO_VIDEO_REJECT:
                rejectAudioToVideo();
                Toast.makeText(AVChatActivity.this, "对方不同意音频切换为视频！", Toast.LENGTH_SHORT).show();
                break;
            ...
            case AVChatControlCommand.NOTIFY_VIDEO_ON:
                ...
                break;
            default:
                Toast.makeText(this, "对方发来指令值：" + notification.getControlCommand(),
                ...
                if (client != null) {
                    String option =
                            ackInfo.getEvent() == AVChatEventType.CALLEE_ONLINE_CLIENT_ACK_AGREE
                                    ? "接听！" : "拒绝！";
                    Toast.makeText(AVChatActivity.this, "通话已在" + client + "端被" + option,
                            Toast.LENGTH_SHORT).show();
                }
                finish();
            }
