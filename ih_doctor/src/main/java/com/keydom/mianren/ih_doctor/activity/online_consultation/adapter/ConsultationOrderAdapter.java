 ...
 * 会诊订单适配器
 */
public class ConsultationOrderAdapter extends BaseEmptyAdapter<InquiryBean> {
    public static final String IS_ORDER = "is_order";
    private static final int NORMAL_VIEW = 0;
    private static final int FOOT_VIEW = 1;

    public ConsultationOrderAdapter(Context context, List<InquiryBean> data) {
        super(data, context);
    }
    ...
    public void bindMyViewHolder(RecyclerView.ViewHolder holder, int position) {
        ((ConsultationOrderAdapter.ViewHolder) holder).bind(position);
    }
            ...
            itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (ImClient.getUserInfoProvider().getUserInfo(mDatas.get(position).getUserCode()) != null) {
                        ...
                        ImClient.startConversation(mContext, mDatas.get(position).getUserCode(),
                                bundle);
                    } else {
                        AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
