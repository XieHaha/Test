 ...
 * @des 会诊室-视频
 */
public class DiagnosisAdviceController extends ControllerImpl<DiagnosisAdviceView> implements View.OnClickListener, View.OnTouchListener {
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.consultation_advice_commit_tv) {
            if (TextUtils.isEmpty(getView().getConsultationAdvice()) && getView().getConsultationVoiceAdvice().size() == 0) {
                ToastUtil.showMessage(mContext, "意见不能为空");
                return;
            }
            if (getView().isOutConsultationDoctor()) {
                showApplyTipsDialog();
            } else {
                diagnosisOrderCreateAdvice();
            }
        } else if (v.getId() == R.id.consultation_advice_refresh) {
            diagnosisOrderAdviceList();
        }
    }
     ...
     * 提交会诊意见
     */
    private void diagnosisOrderCreateAdvice() {
        ApiRequest.INSTANCE.request(HttpService.INSTANCE.createService(ConsultationService.class).diagnosisOrderCreateAdvice(HttpService.INSTANCE.object2Body(getView().getCommitParams())), new HttpSubscriber<String>(getContext(), getDisposable(), true) {
            @Override
            public void requestComplete(@Nullable String data) {
     ...
     * 会诊意见列表
     */
    public void diagnosisOrderAdviceList() {
        ApiRequest.INSTANCE.request(HttpService.INSTANCE.createService(ConsultationService.class).diagnosisOrderAdviceList(getView().getOrderId()), new HttpSubscriber<List<ConsultationAdviceBean>>() {
            @Override
