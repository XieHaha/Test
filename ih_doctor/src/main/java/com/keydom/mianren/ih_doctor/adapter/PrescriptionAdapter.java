...
import com.keydom.mianren.ih_doctor.activity.online_diagnose.DrugChooseActivity;
import com.keydom.mianren.ih_doctor.activity.online_diagnose.DrugUseActivity;
...
public class PrescriptionAdapter extends BaseMultiItemQuickAdapter<MultiItemEntity,
        BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, MultiItemEntity item) {
        switch (helper.getItemViewType()) {
            case TYPE_HEAD:
                ...
                helper.setText(R.id.modal_name,
                        prescriptionHeadBean.getTitleName() + " - " + (prescriptionHeadBean.getIsOutPrescription() == 0 ? "院内" : "外延"));
                select_save.setOnClickListener(new View.OnClickListener() {
                    @SingleClick(1000)
                    @Override
                    public void onClick(View view) {
                        DialogUtils.saveModelDialog(context,
                                context.getTemplateList().get(prescriptionHeadBean.getPosition())
                                , new OnModelDialogListener() {
                                    @Override
                ...
                delete_tv.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        new GeneralDialog(context, "确认删除该药品？", new GeneralDialog.OnCloseListener() {
                            @Override
                            public void onCommit() {
                                context.removeDrug(prescriptionBodyBean.getPosition(),
                                        prescriptionBodyBean.getChildPosition());
                            }
                        }).setTitle("提示").setNegativeButton("取消").setPositiveButton("确认").show();
                    }
                });
                ...
                add_medicine.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        if (context.getSelectList().get(prescriptionBottomBean.getBottomPosition()).size() >= 5) {
                            ...
                            DrugChooseActivity.start(context, list,
                                    prescriptionBottomBean.getBottomPosition(),
                                    String.valueOf(context.getIsOutPrescription()));
                        }
                    }
                });
                break;
